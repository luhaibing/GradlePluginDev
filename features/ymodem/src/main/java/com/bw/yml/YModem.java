...
public class YModem implements FileStreamThread.DataRaderListener {
    ...
    // 新增完成的状态,专门处理 设备将 （ACK,C） 分开发送的情况
    private static final int STEP_FILE_NAME_HALF = 0x02;
    ...
    private static final int MAX_PACKAGE_SEND_ERROR_TIMES = Integer.MAX_VALUE;
    //the timeout interval for a single package
    private static final int PACKAGE_TIME_OUT = Integer.MAX_VALUE;
    ...
    public void stop() {
        ...
        if (currStep != YModem.STEP_END) {
            this.listener.onFailed("stopped");
        }
        CURR_STEP = STEP_HELLO;
    }
        ...
        if (respData != null && respData.length > 0) {
            ...
            switch (CURR_STEP) {
                case STEP_HELLO:
                    handleData(respData);
                    break;
                case STEP_FILE_NAME, STEP_FILE_NAME_HALF:
    ...
    private void handleFileName(byte[] value) {
        if (value.length == 2 && value[0] == ACK && value[1] == ST_C) {//Receive 'ACK C' for file name
            ...
            packageErrorTimes = 0;
            startSendFileData();
        } else if (CURR_STEP == STEP_FILE_NAME && value.length == 1 && value[0] == ACK) {
            Lg.f("Received 'ACK'");
            CURR_STEP = STEP_FILE_NAME_HALF;
        } else if (CURR_STEP == STEP_FILE_NAME_HALF && value.length == 1 && value[0] == ST_C) {
            Lg.f("Received 'C'");
            packageErrorTimes = 0;
            startSendFileData();
    ...
    private void handlePackageFail(String reason) {
        ...
        if (getCurrStep() == STEP_FILE_BODY && packageErrorTimes >= 20 && prevSending != null) {
            previousFrame.set(true);
            sendPackageData(prevSending);
        } else if (packageErrorTimes < MAX_PACKAGE_SEND_ERROR_TIMES) {
            sendPackageData(currSending);
        } else {
    ...
    public boolean isRunning() {
        return CURR_STEP >= STEP_FILE_NAME && CURR_STEP < STEP_COMPLETE;
    }

    public boolean isComplete() {
        return  CURR_STEP == STEP_COMPLETE;
    }
