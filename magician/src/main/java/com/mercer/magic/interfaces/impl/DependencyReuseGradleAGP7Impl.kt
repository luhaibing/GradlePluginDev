...
import com.mercer.magic.ASSEMBLE_TASK_REGEX
 ...
 *   apg7 实现
 */
class DependencyReuseGradleAGP7Impl : OnDependencyReuseWork {
    override fun apply(target: Project) {
        ...
        val assemblePredicate: (String) -> Boolean = {
            it.matches(ASSEMBLE_TASK_REGEX)
        }
        val publishPredicate: (String) -> Boolean = {
            it.matches(PUBLISH_TASK_REGEX)
        }
